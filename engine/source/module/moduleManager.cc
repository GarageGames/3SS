...
bool ModuleManager::unregisterModule( const char* moduleId )
{
    // Fetch modules definitions.
    ModuleDefinitionEntry* pDefinitions = findModuleId( moduleId );
    ...
    if( pDefinitions )
    {
        // Iterate & destroy module defintions
        ModuleDefinitionEntry::iterator definitionItr = NULL;
        do
        {
            definitionItr = pDefinitions->begin();
            if( definitionItr )
            {
                ModuleDefinition* def = *definitionItr;
                clearNotify( def );
                def->deleteObject();
            }
            pDefinitions->erase( definitionItr );
        } while ( definitionItr != pDefinitions->end() );
