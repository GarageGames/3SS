...
#if defined(TORQUE_OS_IOS) || defined(TORQUE_OS_MAC)
#include <ifaddrs.h>
#include <arpa/inet.h>
#endif
...
#ifndef TORQUE_OS_IOS //-Mat no batch files on iPhone (yet?)
//----------------------------------------------------------------
...
//release tests on iPhone is irrelevant.
   ...
   FileTime comModifyTime, scrModifyTime;
   Platform::stringToFileTime("0:0", &comModifyTime);
   Platform::stringToFileTime("0:0", &scrModifyTime);
...
//Also, no DSO generation on iPhone
      ...
      if(false) 
#else
      if(compiled && Platform::compareFileTimes(comModifyTime, scrModifyTime) < 0)
#endif
      {
         // compile this baddie.
		 #if defined(TORQUE_DEBUG)
