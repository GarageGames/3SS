...
bool FileStream::open(const char *i_pFilename, AccessMode i_openMode)
{
   ...
   if( status == File::InvalidPath )
   {
	   // Path doesnt exist, try to create it and re-open the file
	   if( Platform::createPath(i_pFilename) )
	   {
		   status = mFile.open(i_pFilename, (File::AccessMode)i_openMode);
	   }
   }
