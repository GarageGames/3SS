...
void TextureManager::reload( TextureObject* pTextureObject )
{
    // Finish if texture key is invalid.
    if( pTextureObject == NULL )
        return;

	GBitmap* bmp = loadBitmap(pTextureObject->getTextureKey());

	registerTexture(pTextureObject->getTextureKey(), bmp, pTextureObject->getHandleType(), pTextureObject->getClamp());
}
