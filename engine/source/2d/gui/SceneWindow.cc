...
SceneWindow::SceneWindow() :  mpScene(NULL),
                                    ...
                                    mDebugBannerBackgroundColor(0.6f,0.6f,1,0.25f),
                                    mDebugBannerForegroundColor(1,1,1,1),
...
void SceneWindow::setDefaultFont( void )
{
    // Get Debug Font.
#ifdef TORQUE_OS_MAC
...
#ifdef TORQUE_OS_IOS
    mpDebugFont = GFont::create("Arial", 12, GuiControlProfile::sFontCacheDirectory);
#endif
...
void SceneWindow::setDebugBanner( const char* pFontname, const U32 fontSize, const ColorF& foregroundColor, const ColorF& backgroundColor )
{
    // Get Debug Font.
    ...
    if ( mpDebugFont.isNull() )
    {
        ...
        // Warn.
        Con::warnf("SceneWindow::setDebugFont() - Invalid font selected! '%s' (%d). (Using default)", pFontname, fontSize );
    }

    // Set Debug Text/Banner Colours.
    setDebugBannerForegroundColor( foregroundColor );
    setDebugBannerBackgroundColor( backgroundColor );
}
...
void SceneWindow::sendObjectInputEvent( StringTableEntry name, const GuiEvent& event )
{
    ...
    const U32 oldPickCount = mInputEventWatching.size();
...
// Debug Profiling.
    ...
    if ( pScene->getDebugMask() & Scene::SCENE_DEBUG_STATISTICS )
    {
        ...
        glColor4f( mDebugBannerBackgroundColor.red, mDebugBannerBackgroundColor.green, mDebugBannerBackgroundColor.blue, mDebugBannerBackgroundColor.alpha );
        ...
        U32 bannerHeight = (U32)((bannerLineHeight * (F32)mpDebugFont->getHeight()));
        ...
        dglSetBitmapModulation( mDebugBannerForegroundColor );
        ...
        F32 linePositionOffsetY = mpDebugFont->getHeight() * 1.25f;
        ...
        dglDrawText( mpDebugFont, bannerOffset + Point2I(0,(S32)linePositionY), mDebugText, NULL );
