...
{
	// For now I am just putting this in with debug & ignore,  This will need to be properly implemented at some point. see the windows version
   DialogRef alert = _MacCarbCreateAlert(windowTitle, message,CFSTR("Debug"),CFSTR("Ignore"));
   S32 hit = _MacCarbRunAlert(alert);
   return ( hit == kAlertStdAlertOKButton ) ? ALERT_ASSERT_DEBUG : ALERT_ASSERT_IGNORE;
}
